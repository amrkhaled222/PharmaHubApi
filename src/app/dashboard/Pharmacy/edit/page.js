...
export default function EditPharmacy() {
  ...
  useEffect(() => {
    setPharmacyData({
      ...userPharmacy[0],
      Latitude: userPharmacy[0]?.Location?.points[0]?.lat,
      Longitude: userPharmacy[0]?.Location?.points[0]?.lng,
    });
  }, [userPharmacy]);
  const handleCreatePharmacy = async (e) => {
    ...
    try {
      ...
      const response = await api.put("pharmacy", formData, {
        headers: {
      ...
      <form className="grid lg:grid-cols-4 p-4 grid-cols-1  gap-x-6 gap-y-6 mt-4 m-auto w-full ">
        ...
        <div className="lg:row-span-6 lg:col-span-2  col-span-2">
          <MapWithDraggableMarker
            value={{
              lat: pharmcyData.Latitude,
              lng: pharmcyData.Longitude,
            }}
            onChange={setPharmacyData}
          ></MapWithDraggableMarker>
        </div>
